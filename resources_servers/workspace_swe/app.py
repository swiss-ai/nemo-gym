...
class WorkspaceSWEServer(GymnasiumServer):
    ...
    async def _close_endpoint(self, request: Request) -> dict:
    ...
    async def reset(self, metadata: dict, session_id: Optional[str] = None):
        ...
        for sid in list(self.session_state):
            ...
            if state is not None and now - state["created"] > self.config.session_ttl_s:
        ...
        try:
            if metadata.get("repo_url"):
                ...
                if metadata.get("commit"):
                    output, code = await self._run(
                        f"git fetch --depth 1 -- origin {shlex.quote(metadata['commit'])}", root, timeout=180
                    )
                    if code:
                        raise ValueError(f"Fetch failed: {_truncate(output)}")
                    output, code = await self._run("git checkout --detach FETCH_HEAD", root, timeout=180)
                    if code:
            ...
            # Cancellation can arrive during clone, setup, or the initial listing.
            await self.close_session(session_id)
            raise
    ...
    async def step(self, action: NeMoGymResponse, metadata: dict, session_id: Optional[str] = None):
        try:
            return await self._step(action, metadata, session_id)
        except BaseException:
            # A cancelled or failed command ends this episode and releases its workspace.
            await self.close_session(session_id)
            raise

    async def _step(self, action: NeMoGymResponse, metadata: dict, session_id: Optional[str] = None):
